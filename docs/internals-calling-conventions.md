# ACE BASIC Calling Conventions - Internal Documentation

This document describes the internal calling conventions used by ACE for SUBs and FUNCTIONs, including the CSLR (BASIC) and C conventions, and all SUB modifier keywords.

## Overview

ACE implements two distinct calling conventions:

| Convention | Usage | Parameters | Stack Cleanup | Return Value |
|------------|-------|------------|---------------|--------------|
| **CSLR (BASIC)** | ACE SUBs | Callee's stack frame | Automatic | Frame var or D0 |
| **C** | External/library functions | Caller's stack | Caller cleans up | D0 |

---

## CSLR Calling Convention (ACE BASIC Default)

CSLR stands for "CLoSuRe" - the magic signature `$434C5352` (ASCII "CLSR") identifies functions using this convention.

### Characteristics

- **Parameters location**: Stored in the **callee's stack frame** at negative offsets from SP
- **Multitasking safety**: Caller calls `Forbid()` before pushing parameters; callee calls `Permit()` in prologue
- **Return value**: Stored in frame variable (normal SUBs) or D0 (EXTERNAL/INVOKABLE/CALLBACK)
...
Used for external functions, library calls, and indirect calls to unknown function signatures.

### Characteristics

- **Parameters location**: Pushed on **caller's stack** in reverse order (rightmost first)
...
ACE supports three mutually exclusive modifier keywords for SUBs:
...
**Why Needed:** Frame variables at `-X(a5)` are not accessible to indirect callers because the callee's A5 is different. Using D0 ensures the return value is accessible.
...
## Return Value Handling Summary

| SUB Type | Return Location | How Caller Accesses |
|----------|-----------------|---------------------|
| Normal (frame offset) | Frame var at `-X(a5)` | Via stack after return |
...
The compiler determines the mechanism by checking the `address` field in the symbol table:

```c
if (storage_item->address != extfunc)
    /* Normal SUB - return via frame variable */
else
    /* EXTERNAL/INVOKABLE/CALLBACK - return in D0 */
```

---

## Runtime CLSR Detection (INVOKE)

When calling through a function pointer via `INVOKE`, the compiler generates code to detect the calling convention at runtime:
...
| Value | Meaning | Usage |
|-------|---------|-------|
| Negative number | Stack frame offset | Normal SUBs access locals/params via `-X(a5)` |
| `extfunc` (3004) | External/special marker | EXTERNAL, CALLBACK, INVOKABLE, Module SUBs |
| Positive number | Library function offset | Shared library functions offset from base register |

---

## Module Considerations

When compiling with `-m` (module mode), all SUBs automatically use D0 for return values:

```c
if (module_opt)
    sub_ptr->address = extfunc;  /* Force D0 return */
```

This is necessary because modules cannot use normal A4-relative addressing for the main program's frame.
